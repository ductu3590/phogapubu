...
2. Đăng nhập đúng tài khoản **chủ quán Bảo Lương** và mở hai tab:

   - POS: [http://localhost:3000/admin/cashier](http://localhost:3000/admin/cashier)
   - Kitchen: [http://localhost:3000/kitchen/bia-lau-bao-luong](http://localhost:3000/kitchen/bia-lau-bao-luong)
...
4. POS mở tab phiếu. Trên Kitchen xuất hiện một thẻ món đúng revision vừa duyệt. F5 Kitchen vẫn giữ đúng món đó.
...
6. Bấm lại hoặc refresh POS: không tạo phiếu gốc/chuông/batch Kitchen thứ hai. Nếu muốn in thêm, dùng **In lại / điều chỉnh**; không dùng nút xác nhận đơn thông thường.

PASS nếu owner duyệt đúng một revision, Kitchen chỉ nhận món đã duyệt, không có đường xác nhận lách revision.
...
3. Phiếu mới phải đề **PHIẾU ĐIỀU CHỈNH** và `v2`; Kitchen phải dùng món của v2 sau khi duyệt, không thay đổi ngay từ lúc khách sửa.
...
1. Với preorder chưa in, ở `/admin/reservations` bấm **Khách đã đến**: POS mở đúng bill/mâm; không tự sinh phiếu hay thêm thẻ bếp.
