...
>    prod.** Spec này KHÔNG nghiệm thu hành vi đó; PM-1 của
>    [spec multi-method](2026-07-15-multi-method-payment-design.md) sẽ vá.
...
7. Tại quầy: CASH thu đủ tiền; `bank_transfer` chỉ xác nhận sau khi thấy tiền thực sự vào tài khoản quán.
...
- CASH/`bank_transfer` vào bếp ngay nhưng chưa tính doanh thu.
...
|---|---|---|---|
| `zalopay` | Khách tự thanh toán qua **ví** Zalo Checkout SDK | Sau callback thành công (`resultCode`) | Có `payment_received_at` |
| `cash` | Tiền mặt trả sau tại quầy | Ngay khi staff tạo | Quầy xác nhận đã nhận tiền |
| `bank_transfer` | Chuyển khoản thẳng vào TK ngân hàng quán | Ngay khi staff tạo | Quầy/bếp xác nhận đã nhận tiền |

#### ⚠️ Chuyển khoản qua Zalo KHÔNG phải một phương thức riêng

**Sửa 2026-07-15 — bản trước của mục này SAI và đã bị gỡ.**

Bản trước khai `zalopay` "gồm cả chuyển khoản qua Zalo (`method=BANK`)", vào bếp "sau
callback/**notify** thành công", doanh thu tính theo `zalopay_trans_id`. Cả ba đều sai:

Notify URL của Zalo chỉ là sự kiện **"khách vừa CHỌN phương thức"** — Zalo xếp nó chung nhóm
với COD vì **Zalo không giữ tiền chuyển khoản**. Payload không có `resultCode`/`amount`. Đã
kiểm chứng thực tế: thoát app ngân hàng không chuyển tiền, đơn vẫn confirmed + vẫn vào doanh
thu. **Đó là bug, không phải hành vi đúng.**

Zalo Checkout không phải phương thức thanh toán — nó chỉ là **một cách khởi tạo** (deeplink mở
app ngân hàng). Tiền đi cùng một đường bank→bank như QR ở quầy. Nên chỉ có **một**
`bank_transfer`.

📄 Chẩn đoán đầy đủ + cách vá: [2026-07-15-multi-method-payment-design.md](2026-07-15-multi-method-payment-design.md) §1, §2.

**Ràng buộc thứ tự:** spec multi-method yêu cầu SA-1…SA-5 PASS trước. Nếu spec này vẫn nghiệm
thu hành vi sai thì hai tài liệu tạo phụ thuộc vòng. Vì vậy mục này và checklist §11 đã sửa
**ngay bây giờ**, trước SA-1 — không đợi tới PM-1.

Trong phạm vi spec staff: đơn `bank_transfer` **chưa có cơ chế xác nhận tự động**, nên tiền chỉ
được ghi nhận khi owner bấm xác nhận (§6.2). PM-1 sẽ vá `checkout-notify`; PM-3 thêm bếp xác
nhận; PM-5 thêm SePay.
...
Cột này là nguồn dữ liệu cho danh sách phương thức mà **khách** thấy trong mini-app. Bật
`bank_transfer` mà không chặn gì thì mini-app hiện luôn nút đó cho khách tự order.

**Trong phạm vi spec staff, `bank_transfer` là staff-only** — không phải vì bản chất, mà vì
khách chưa có đường xác nhận tiền nào (bếp xác nhận đến PM-3, SePay đến PM-5). Cho khách
chọn bây giờ là mở lại đúng lỗ hổng §3.1.

Quy tắc tạm thời tới hết SA-5:

- Mini-app whitelist `['zalopay','cash']`, không phải blacklist.
- `create_order` (RPC khách) **chỉ nhận `zalopay`/`cash`**; chỉ `staff_create_order` nhận
  `bank_transfer`. Xem §6.1 bước 5.

**PM-4 sẽ mở `bank_transfer` cho khách** sau khi PM-1 vá notify và PM-3 có bếp xác nhận. Lúc
đó quy tắc này hết hiệu lực — đừng cứng hoá nó thành bất biến vĩnh viễn trong code.
...
- CASH/`bank_transfer` đã thanh toán khi `payment_received_at is not null`.
...
```text
Đơn staff (cash / bank_transfer) — vào bếp ngay từ pending:
  pending ─────────────────────────► cooking ──► ready

Đơn khách (zalopay — VÍ) — vào bếp sau khi có tiền:
  pending ──[callback resultCode=1]──► confirmed ─► cooking ──► ready
...
- Đơn staff CASH/`bank_transfer`: `pending` → `cooking` → `ready` (vào bếp ngay từ `pending`, §8.1).

Thanh toán độc lập bằng `payment_received_at`. Một đơn có thể `ready` nhưng chưa thanh toán, hoặc `cooking` nhưng đã thanh toán. Code mới không đổi CASH/`bank_transfer` thành `paid`; dữ liệu legacy `cash + paid` vẫn được đọc tương thích.

### 4.4 Doanh thu thực nhận

```text
ZaloPay (VÍ, callback resultCode=1): zalopay_trans_id != null và không cancelled
CASH/bank_transfer:            payment_received_at != null và không cancelled
...
|---|---|---|
| `get_daily_revenue()` — `014_cashless_default_revenue.sql` | hardcode `cash AND status='paid'` | thêm nhánh `payment_received_at`, thêm `bank_transfer` |
| `get_daily_revenue().cash_pending` — cùng file | `cash AND status NOT IN ('paid','cancelled')` | đổi sang `payment_received_at is null`, gồm cả `bank_transfer` (nên đổi tên → `manual_pending`) |
...
Staff được đọc quán tối thiểu, bàn active, menu/topping đang bán và đơn đúng quán; được tạo CASH/`bank_transfer` và theo dõi realtime.
...
5. Chỉ nhận `cash` hoặc `bank_transfer` — **không nhận `zalopay`** (staff không thu hộ ZaloPay).
   Ngược lại, `create_order` (RPC khách) **không được nhận `bank_transfer`**: whitelist hai
...
- Chỉ CASH/`bank_transfer`, không cancelled. Đơn `zalopay` (ví) bị từ chối: tiền đã do callback
  của Zalo xác nhận thật, không xác nhận tay.
...
- Checkout xác nhận lại bàn, có hai nút lớn CASH/`bank_transfer`; không ZaloPay/voucher.
...
- CASH/`bank_transfer` chỉ có “tiền thật” sau `payment_received_at`.
...
### Sprint SA-1 — Database, `bank_transfer`, role và RPC
...
- CASH/`bank_transfer` checkout, sticky cart, loading/error/retry.
...
- Kitchen nhận CASH/`bank_transfer` live.
...
- UI xác nhận CASH/`bank_transfer`, audit người/thời gian.
...
3. Lặp lại `bank_transfer`; Kitchen xử lý giống CASH, badge đúng.
...
14. **Mini-app khách KHÔNG hiện `bank_transfer`** kể cả khi quán đã bật (§4.1) — tới hết SA-5,
    khách chưa có đường xác nhận tiền nào.
15. ~~Khách tự order + chuyển khoản qua Zalo vẫn vào doanh thu~~ — **ĐÃ GỠ 2026-07-15.**
    Mục này từng bắt nghiệm thu rằng đơn BANK-qua-Zalo phải vào doanh thu, tức là **nghiệm thu
    chính cái bug** §3.1 (thoát app ngân hàng vẫn được tính tiền). Không thay bằng test khác ở
    spec này: chuyển khoản của khách nằm ngoài phạm vi SA, và PM-1 sẽ test đúng chiều ngược lại
    (*thoát app ngân hàng → KHÔNG vào doanh thu*).
...
admin-web/lib/kitchen-announce.ts          ← :10 mở cho bank_transfer
