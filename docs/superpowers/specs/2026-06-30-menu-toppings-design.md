...
CREATE INDEX idx_menu_item_toppings_lookup
...
export const getItemUnitPrice = (i: { price: number; selectedToppings?: {price:number}[] }) =>
  i.price + (i.selectedToppings ?? []).reduce((s, t) => s + t.price, 0);
export const getItemLineTotal = (i: { quantity: number; ... }) =>
  getItemUnitPrice(i) * i.quantity;
```

Call-site BẮT BUỘC sửa (đã verify trên codebase):
- `mini-app/src/pages/order-status/index.tsx:286` — hiện `item.price * item.quantity` → dùng `getItemLineTotal`.
...
- `menu_item_toppings`: bật RLS.
  ...
  - **INSERT/UPDATE/DELETE**: chỉ service-role (admin web dùng `createAdminClient`) — khớp cách `menu_items` được ghi qua admin client, không mở cho anon.
