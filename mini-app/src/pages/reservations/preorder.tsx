...
export default function ReservationPreorderPage() {
  ...
  const drafts = usePreorderCartStore((s) => s.drafts); const add = usePreorderCartStore((s) => s.add); const updateQuantity = usePreorderCartStore((s) => s.updateQuantity); const items = drafts[`${storeId}:${reservationId}`]?.items ?? [];
  ...
  return <div className="min-h-full bg-[#F7F8FA] pb-24">
    ...
    {menu.map((category) => <section key={category.id} className="mt-3 bg-white"><h2 className="border-b border-neutral100 px-4 py-3 text-large-m font-semibold">{category.name}</h2>{category.products.map((p) => { const line = items.find((x) => x.productId === p.id && !x.variant && x.selectedVariants.length === 0); return <div key={p.id} className="flex w-full items-center justify-between border-b border-neutral100 px-4 py-3"><button disabled={!p.isAvailable} onClick={() => addProduct(p)} className="text-left disabled:opacity-50"><span className="block text-normal-sb">{p.name}</span><span className="text-small text-primary">{formatCurrency(p.price)}đ</span></button>{line && !(p.variants.length || p.toppings.length) ? <span className="flex items-center gap-3"><button onClick={() => updateQuantity(storeId, reservationId, line.id, line.quantity - 1)} className="h-8 w-8 rounded-full border border-primary text-primary">−</button><b>{line.quantity}</b><button onClick={() => updateQuantity(storeId, reservationId, line.id, line.quantity + 1)} className="h-8 w-8 rounded-full bg-primary text-white">+</button></span> : <button disabled={!p.isAvailable} onClick={() => addProduct(p)} className="rounded-full bg-primary px-3 py-1 text-small font-bold text-white disabled:opacity-50">+</button>}</div>})}</section>}
