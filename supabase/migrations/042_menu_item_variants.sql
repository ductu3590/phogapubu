...
-- Rerun-safe: file này chỉ có DDL bảng/cột/trigger/RLS. Phần CREATE OR REPLACE của
-- create_order/staff_create_order nằm ở 043 và CỐ Ý không để chung ở đây — chạy lại
-- 042 không được phép kéo hai RPC sống lùi về bản cũ.
--
...
-- ⚠️ HỢP ĐỒNG HIỂN THỊ — đọc trước khi vẽ bất cứ chỗ nào có biến thể:
--   • item_name là chuỗi hiển thị DUY NHẤT. create_order đã ghép sẵn 'Bia hơi (Cốc)'.
--     Phiếu bếp, loa đọc đơn TTS, bill, lịch sử đơn — tất cả đọc item_name và KHÔNG
--     phải sửa gì để hỗ trợ biến thể.
--   • variant_name CHỈ để thống kê/lọc (bán bao nhiêu Cốc so với Tháp).
--     KHÔNG BAO GIỜ vẽ nó cạnh item_name — sẽ ra 'Bia hơi (Cốc) — Cốc'.
--   • item_price đã LÀ giá biến thể, không phải giá món. Không cộng thêm gì.
