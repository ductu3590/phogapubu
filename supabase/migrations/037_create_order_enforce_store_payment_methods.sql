-- 037_create_order_enforce_store_payment_methods.sql
-- create_order (v9) = 034 v8 + CHAN phuong thuc khong nam trong stores.payment_methods.
--
-- Vi sao can: mini-app CU dang publish loc payment_methods theo ten cu ('zalopay'|'cash');
-- sau rename (mig 032) DB tra 'zalo_checkout' -> loc ra rong -> mini-app cu roi vao fallback
-- ['zalopay','cash'] va HIEN nut tien mat du quan da tat. Server truoc day khong kiem tra
-- cau hinh quan nen don 'cash' VAN VAO THAT (da co 3 don customer_zalo ngay 2026-07-27).
--
-- Client luon co the cu (khach chua mo lai app, Zalo cache ban cu) -> chot chan phai o server.
-- Luong nhan vien dat ho dung RPC RIENG (staff_create_order) nen KHONG bi anh huong.
...
BEGIN
  ...
  -- Chan phuong thuc quan da tat (vd tien mat): khong tin danh sach ma client hien ra.
  IF NOT EXISTS (
    SELECT 1 FROM stores
     WHERE id = p_store_id AND p_payment_method = ANY(payment_methods)
  ) THEN
