...
| 2026-07-01 | **Thêm MEVO Internal Backend trong `admin-web` tại `/mevo`**; dùng chung `/login`, phân quyền bằng `mevo_operators.role + store_id` | MEVO cần cockpit nội bộ để tạo/quản lý quán và cấu hình mini-app/Zalo OA/ZaloPay theo từng app. `mevo_superadmin` có `store_id = NULL` vào `/mevo`; `store_owner` bắt buộc có `store_id` vào `/admin`; user không có operator row không vào được. Chi tiết: [docs/superpowers/specs/2026-07-01-mevo-internal-backend-onboarding-cockpit-design.md](docs/superpowers/specs/2026-07-01-mevo-internal-backend-onboarding-cockpit-design.md) |
| 2026-07-15 | **Staff Assisted Ordering**: nhân viên chỉ đặt hộ khi khách không thể tự dùng MEVO; chọn bàn/món và CASH hoặc BANK, khách thanh toán sau tại quầy; mọi màn hình vận hành realtime, không refresh | Giữ self-order là luồng mặc định nhưng bao phủ khách không có/không biết dùng Zalo. CASH/BANK vào bếp ngay, chỉ tính doanh thu sau khi quầy xác nhận tiền thật. Thêm `store_staff`, audit người tạo/người nhận tiền và tách trạng thái thanh toán khỏi tiến độ bếp. Chi tiết: [docs/superpowers/specs/2026-07-15-staff-assisted-ordering-design.md](docs/superpowers/specs/2026-07-15-staff-assisted-ordering-design.md) |
| 2026-08-31 | **Sprint 1 (mig 039)** — tách `stores.payment_timing` (`prepay`/`postpay`) khỏi `payment_method`; bảng `table_sessions` (một bàn đang có khách = một phiên = một bill, khoá theo chủ phiên khớp **Zalo UID HOẶC device id**); `orders.session_id`; `create_order` v10; màn `/staff/tables` chốt bill. `orderInKitchen` đọc `payment_timing`, KHÔNG đọc `payment_method` nữa | Trục "KHI NÀO thu tiền" trước đây bị suy ra từ trục "thu BẰNG GÌ" nên quán trả trước bật tiền mặt cho nhân viên thì đơn QR tiền mặt vẫn lọt vào bếp. ⚠️ Sprint 2 (lớp Mâm) chỉ cần sửa hàm `open_session_id_for_table()` |
...
| 2026-09-17 | **BL-0 hoàn tất nền cấu hình workflow theo quán**: `store_workflow_settings` + audit, preset Pubu/Bảo Lương, capability được chặn cả Mini App lẫn trigger DB, POS release theo quán, queue Gọi nhân viên qua RPC và quyền close bill owner-only. | Cho phép các quán sau chọn mô hình vận hành trong Admin Web mà không hồi quy Pubu. Form đặt bàn/đặt món trước vẫn thuộc BL-3. Zalo App ID Bảo Lương `671794256689452743` đã tồn tại nhưng xác minh/phê duyệt chưa rõ; OA Bảo Lương chưa đăng ký. |
| 2026-09-19 | **Tách BL-2 thành BL-2A/2B/2C**: BL-2A làm ngay hàng đợi đặt bàn trên POS/Admin Mobile; BL-2B dành cho onboarding/thông báo OA khi có credential; BL-2C làm hàng đợi duyệt/in preorder cùng hoặc sau BL-3. | POS đặt bàn không bị chặn bởi OA chưa đăng ký hoặc preorder phía khách chưa tồn tại; mỗi phần có checkpoint test riêng và vẫn dùng chung reservation source of truth từ BL-1. |
| 2026-09-22 | **BL-2B bỏ Zalo OA Open API khỏi đường găng**: cảnh báo booking nội bộ pilot đi qua relay `zca-js` đến nhóm Zalo vận hành riêng của quán; OA/ZNS là tùy chọn trong tương lai. | OA Open API yêu cầu gói khoảng 2,5 triệu/tháng, không phù hợp pilot. `zca-js` là API không chính thức nên chỉ được dùng best-effort với bot riêng, outbox/audit/fallback POS và gọi điện vẫn là nguồn vận hành an toàn. |
| 2026-09-23 | **Bảo Lương không dùng Kitchen Display**: mọi đơn, kể cả món đặt trước, chỉ xuống bếp bằng phiếu giấy do chủ quán duyệt/in từ POS. Hai liên cùng có giá và tổng tiền; khác nhãn để tránh phát nhầm. | POS là nguồn quyết định tại quầy; không tạo màn bếp song song gây trùng thông tin hoặc sai thao tác. Lời mời khách chọn món trước thuộc BL-3 Task 7, sau khi booking đã được owner xác nhận. |
| 2026-09-23 | **Bảo Lương khóa món đặt trước ngay sau khi khách gửi**: khách không sửa/hủy hoặc tạo batch đặt trước thứ hai; chỉ gọi thêm sau khi đã đến quán và mở phiên QR/bill. | Snapshot sớm giúp quán quản lý đơn giản; server vẫn idempotent khi khách retry do lỗi mạng. Mini App phải báo: “Đã gửi món đặt trước. Món đã chốt, vui lòng gọi thêm tại quán nếu cần.” |
| 2026-09-24 | **Bàn đã xác nhận của Bảo Lương được ưu tiên từ 60 phút trước giờ đến**: POS đánh dấu/khóa bàn trống; QR không mở phiên mới trong khoảng này. | Tránh khách vãng lai ngồi vào bàn gần giờ đoàn đã đặt. Phiên đã mở từ trước không bị hệ thống tự đóng; chủ quán xử lý thực tế. |
